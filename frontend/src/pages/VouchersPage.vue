...
async function printVoucher(row: VoucherRow) {
  ...
  try {
    ...
    const issuedDate = new Date(row.createdAt).toLocaleDateString(undefined, { day: '2-digit', month: 'short', year: 'numeric' })
    const expiryDate = row.expiresAt
      ? new Date(row.expiresAt).toLocaleDateString(undefined, { day: '2-digit', month: 'short', year: 'numeric' })
      : 'No expiry'
    const statusLabel = row.status.charAt(0) + row.status.slice(1).toLowerCase()
    const statusClass = row.status.toLowerCase()
    printWindow.document.write(`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Voucher ${escapePrintValue(row.voucherCode)}</title><style>
      @import url('https://fonts.googleapis.com/css2?family=Outfit:wght@400;600;700;800&display=swap');@page{size:A4 portrait;margin:14mm}*{box-sizing:border-box}body{font-family:Outfit,sans-serif;color:#0f172a;margin:0;padding:32px;background:#fff;font-variant-numeric:tabular-nums}.voucher{width:100%;max-width:720px;margin:auto;background:#fff;border:1.5px solid #0f766e;border-radius:14px;overflow:hidden}.voucher-header{display:flex;align-items:flex-start;justify-content:space-between;gap:24px;padding:22px 26px;border-bottom:1px solid #dceae8}.organisation-name{max-width:420px;color:#0f766e;font-size:25px;font-weight:800;letter-spacing:-.03em;line-height:1.15;overflow-wrap:anywhere}.document-type{text-align:right}.document-type strong{display:block;font-size:17px;line-height:1.2}.status{display:inline-flex;align-items:center;margin-top:8px;padding:5px 9px;border-radius:999px;background:#fff7df;color:#854d0e;font-size:10px;font-weight:800;letter-spacing:.08em;text-transform:uppercase}.status.redeemed{background:#dcfce7;color:#166534}.status.void{background:#fee2e2;color:#991b1b}.voucher-body{display:grid;grid-template-columns:minmax(0,1fr) 238px}.details{display:flex;flex-direction:column;justify-content:center;padding:30px 28px}.beneficiary-label,.field dt{color:#64748b;font-size:10px;font-weight:800;letter-spacing:.08em;text-transform:uppercase}.beneficiary{padding-bottom:22px;border-bottom:1px solid #e2e8f0}.beneficiary-name{margin-top:6px;font-size:27px;font-weight:800;line-height:1.2;letter-spacing:-.02em}.fields{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:20px 24px;margin:22px 0 0}.field{margin:0;min-width:0}.field dd{margin:5px 0 0;color:#334155;font-size:13px;font-weight:600;line-height:1.35;overflow-wrap:anywhere}.redeem-panel{position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:26px 22px;background:#fff;border-left:1px dashed #7dbeb8;text-align:center}.redeem-panel:before,.redeem-panel:after{content:"";position:absolute;left:-8px;width:15px;height:15px;border:1.5px solid #0f766e;border-radius:50%;background:#fff}.redeem-panel:before{top:-8px}.redeem-panel:after{bottom:-8px}.qr{display:block;width:182px;height:182px;padding:7px;background:#fff;border:1px solid #b9d8d4;border-radius:8px}.scan{margin-top:12px;font-size:12px;font-weight:800}@media(max-width:620px){body{padding:12px}.voucher-body{grid-template-columns:1fr}.voucher-header{padding:19px 20px}.organisation-name{max-width:230px;font-size:21px}.details{padding:25px 20px}.redeem-panel{border-top:1px dashed #7dbeb8;border-left:0;padding:22px}.redeem-panel:before,.redeem-panel:after{top:-8px}.redeem-panel:before{left:-8px}.redeem-panel:after{right:-8px;left:auto}}@media print{body{padding:0;background:#fff;-webkit-print-color-adjust:exact;print-color-adjust:exact}.voucher{break-inside:avoid}}
    </style></head><body><section class="voucher" aria-label="Payment voucher"><header class="voucher-header"><div class="organisation-name">${escapePrintValue(titleCaseWords(orgName(row.organisationCode)))}</div><div class="document-type"><strong>Payment voucher</strong><span class="status ${escapePrintValue(statusClass)}">${escapePrintValue(statusLabel)}</span></div></header><div class="voucher-body"><main class="details"><div class="beneficiary"><div class="beneficiary-label">Issued to</div><div class="beneficiary-name">${escapePrintValue(row.householdName)}</div></div><dl class="fields"><div class="field"><dt>Household code</dt><dd>${escapePrintValue(row.householdNumber)}</dd></div><div class="field"><dt>Purpose</dt><dd>${escapePrintValue(row.purpose || 'General assistance')}</dd></div><div class="field"><dt>Date issued</dt><dd>${escapePrintValue(issuedDate)}</dd></div><div class="field"><dt>Valid until</dt><dd>${escapePrintValue(expiryDate)}</dd></div></dl></main><aside class="redeem-panel"><img class="qr" src="${qrImage}" alt="Voucher QR code"><div class="scan">Scan to verify and redeem</div></aside></div></section><script>window.onload=()=>{window.focus();window.print()}<\/script></body></html>`)
