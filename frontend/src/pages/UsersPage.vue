...
interface UserRow{id:number;email:string;username:string;firstName?:string;surname?:string;partnerCode?:string;anchorId?:number;anchorName?:string;userScope:string;roleId?:number;roleName?:string;status:number;createdAt:string;systemAdmin?:boolean}
 ...
 <div class="admin-page">
  ...
  <v-card border flat class="admin-card"><div class="table-tools"><v-text-field v-model="search" prepend-inner-icon="mdi-magnify" label="Search users" hide-details density="compact" variant="outlined"/><v-select v-model="roleFilter" :items="roleFilterOptions" label="Filter by role" clearable hide-details density="compact" variant="outlined" class="role-filter"/><span>{{ filteredUsers.length }} accounts</span></div>
   <v-data-table :headers="headers" :items="filteredUsers" :search="search" :loading="loading">
    ...
    <template #item.actions="{item}"><v-btn v-if="auth.can('ACCESS_USERS')" size="small" variant="text" icon="mdi-history" :aria-label="`View ${item.email} activity history`" @click="openHistory(item)"/><v-btn v-if="auth.can('ACCESS_USERS')" size="small" variant="text" icon="mdi-pencil-outline" aria-label="Edit user" class="mr-1" @click="openEdit(item)"/><v-btn v-if="auth.can('ACCESS_USERS')" size="small" variant="text" :icon="item.status===1?'mdi-account-cancel-outline':'mdi-account-check-outline'" :color="item.status===1?'error':'success'" :aria-label="`${item.status===1?'Deactivate':'Activate'} ${item.email}`" @click="toggle(item)"/></template>
   </v-data-table>
  </v-card>
