    ...
    <v-dialog v-model="dialog" max-width="480">
      <v-card>
        ...
        <v-card-text>
          ...
          <v-select v-if="dialogLevel === 'LOCATION' || dialogLevel === 'VILLAGE'" v-model="form.countyCode" :items="countiesForState(form.stateCode)" item-title="name" item-value="code" label="County" :disabled="editing" />
          <v-select v-if="dialogLevel === 'VILLAGE'" v-model="form.locationCode" :items="locationsForCounty(form.countyCode)" item-title="name" item-value="code" label="Location" :disabled="editing" />
          <v-text-field v-model="form.name" label="Name" placeholder="e.g. Central" class="mt-2" />
          <p class="text-caption text-medium-emphasis mt-2 mb-0">
            {{ editing ? `Code ${editingCode} and its parent hierarchy remain unchanged.` : 'The code is generated automatically. State is the top-level geography, so no separate country field is needed.' }}
          </p>
        </v-card-text>
        <v-card-actions>
          <v-spacer />
