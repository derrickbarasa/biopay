...
const form = ref({ name: '', stateCode: '', countyCode: '', locationCode: '' })
...
async function loadAll() {
  if (auth.isSystemAdmin && !selectedAnchorId.value) {
    states.value = []; counties.value = []; locations.value = []; villages.value = []
    return
  }
  loading.value = true
  try {
...
async function save() {
  ...
  try {
    ...
    const res = await dispatch<{ code: string }>(levelToCode[dialogLevel.value], scoped(payload))
...
async function remove(level: typeof dialogLevel.value, code: string) {
  ...
  try {
    await dispatch('DELETE_GEO_NODE', scoped({ level, code }))
...
<template>
  <div>
    ...
    <v-select
      ...
      label="Manage locations for anchor" variant="outlined" density="comfortable" class="mb-4" style="max-width: 420px"
    />
    <v-alert v-if="auth.isSystemAdmin && !selectedAnchorId" type="info" variant="tonal" class="mb-4">
      Choose an anchor to view or change its location hierarchy.
    </v-alert>
          ...
          <v-card-text>
            ...
            <v-data-table :headers="stateHeaders" :items="states" :search="tableSearch" :loading="loading" density="comfortable">
              <template #item.actions="{ item }">
                <v-btn v-if="auth.can('ACCESS_LOCATIONS')" icon="mdi-delete" size="small" variant="text" color="error" :aria-label="`Delete state ${item.name}`" @click="remove('STATE', item.code)" />
              </template>
        ...
        <v-window-item value="counties">
          <v-card-text>
            ...
            <v-data-table :headers="countyHeaders" :items="counties" :search="tableSearch" :loading="loading" density="comfortable">
              ...
              <template #item.actions="{ item }">
                <v-btn v-if="auth.can('ACCESS_LOCATIONS')" icon="mdi-delete" size="small" variant="text" color="error" :aria-label="`Delete county ${item.name}`" @click="remove('COUNTY', item.code)" />
              </template>
        ...
        <v-window-item value="locations">
          <v-card-text>
            ...
            <v-data-table :headers="locationHeaders" :items="locations" :search="tableSearch" :loading="loading" density="comfortable">
              ...
              <template #item.actions="{ item }">
                <v-btn v-if="auth.can('ACCESS_LOCATIONS')" icon="mdi-delete" size="small" variant="text" color="error" :aria-label="`Delete location ${item.name}`" @click="remove('LOCATION', item.code)" />
              </template>
        ...
        <v-window-item value="villages">
          <v-card-text>
            ...
            <v-data-table :headers="villageHeaders" :items="villages" :search="tableSearch" :loading="loading" density="comfortable">
              ...
              <template #item.actions="{ item }">
                <v-btn v-if="auth.can('ACCESS_LOCATIONS')" icon="mdi-delete" size="small" variant="text" color="error" :aria-label="`Delete village ${item.name}`" @click="remove('VILLAGE', item.code)" />
              </template>
