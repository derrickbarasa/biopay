/**
 * Every processingCode the backend's two dispatch routes accept. Public
 * codes go through POST /biopay/authentication (no Authorization header);
 * everything else goes through the JWT-protected POST /biopay/api/v1/req.
 * Mirrors com.biopay.main.EntryPoint's `publicCodes` array exactly.
 */
export const PUBLIC_PROCESSING_CODES = new Set([
  'LOGIN_USER',
  'LOGIN_SUPERVISOR',
  'REFRESH_TOKEN',
  'REQUEST_LOGIN_OTP',
  'VERIFY_LOGIN_OTP',
  'SIGNUP_ANCHOR',
  'REQUEST_PASSWORD_RESET',
  'RESET_PASSWORD',
  'GET_EMAIL_APPROVAL',
  'CONFIRM_EMAIL_APPROVAL',
  'API_TOKEN',
] as const)

export type ProcessingCode =
  // auth (public)
  | 'LOGIN_USER'
  | 'LOGIN_SUPERVISOR'
  | 'REFRESH_TOKEN'
  | 'REQUEST_LOGIN_OTP'
  | 'VERIFY_LOGIN_OTP'
  | 'SIGNUP_ANCHOR'
  | 'REQUEST_PASSWORD_RESET'
  | 'RESET_PASSWORD'
  | 'GET_EMAIL_APPROVAL'
  | 'CONFIRM_EMAIL_APPROVAL'
  | 'API_TOKEN'
  // auth (authenticated)
  | 'LOGOUT'
  | 'CHANGE_PASSWORD'
  | 'UPDATE_PROFILE'
  | 'ME'
  | 'TOTP_SETUP_INIT'
  | 'TOTP_SETUP_CONFIRM'
  | 'TOTP_DISABLE'
  | 'EMAIL_OTP_ENABLE'
  | 'EMAIL_OTP_DISABLE'
  // administration
  | 'GET_ANCHORS'
  | 'CREATE_ANCHOR'
  | 'UPDATE_ANCHOR'
  | 'TOGGLE_ANCHOR_STATUS'
  | 'GET_USERS'
  | 'GET_USER'
  | 'CREATE_USER'
  | 'UPDATE_USER'
  | 'TOGGLE_USER_STATUS'
  | 'UNBLOCK_USER'
  | 'RESET_USER_PASSWORD'
  | 'GET_AUDIT_LOGS'
  | 'GET_API_CLIENTS'
  | 'CREATE_API_CLIENT'
  | 'TOGGLE_API_CLIENT_STATUS'
  | 'GET_ROLES'
  | 'SAVE_ROLE'
  | 'DELETE_ROLE'
  | 'GET_PERMISSIONS'
  | 'CREATE_PERMISSION'
  | 'DELETE_PERMISSION'
  // organizations
  | 'CREATE_ORGANIZATION'
  | 'UPDATE_ORGANIZATION'
  | 'TOGGLE_ORGANIZATION_STATUS'
  | 'GET_ORGANIZATION'
  | 'GET_ORGANIZATIONS'
  // officers
  | 'CREATE_OFFICER'
  | 'UPDATE_OFFICER'
  | 'TOGGLE_OFFICER_STATUS'
  | 'ASSIGN_OFFICER_LOCATION'
  | 'GET_OFFICER'
  | 'GET_OFFICERS'
  | 'UNBLOCK_OFFICER'
  | 'RESET_OFFICER_PASSWORD'
  // households / alternates
  | 'CREATE_HOUSEHOLD'
  | 'UPDATE_HOUSEHOLD'
  | 'DEACTIVATE_HOUSEHOLD'
  | 'ACTIVATE_HOUSEHOLD'
  | 'GET_HOUSEHOLD'
  | 'GET_HOUSEHOLDS'
  | 'GET_HOUSEHOLD_HISTORY'
  | 'GET_HOUSEHOLD_VOUCHER'
  | 'GET_HOUSEHOLD_LOCATIONS'
  | 'CHECK_HOUSEHOLD_DUPLICATE'
  | 'BULK_UPLOAD_HOUSEHOLDS'
  | 'SET_HOUSEHOLD_REVIEW_STATUS'
  | 'BULK_SET_HOUSEHOLD_REVIEW_STATUS'
  | 'CREATE_ALTERNATE'
  | 'UPDATE_ALTERNATE'
  | 'DELETE_ALTERNATE'
  | 'ACTIVATE_ALTERNATE'
  | 'GET_ALTERNATES'
  // organisation modules
  | 'GET_ORGANIZATION_MODULES'
  | 'UPDATE_ORGANIZATION_MODULES'
  // subscription
  | 'GET_SUBSCRIPTION'
  | 'GET_ALL_SUBSCRIPTIONS'
  | 'RENEW_SUBSCRIPTION'
  | 'SET_SUBSCRIPTION_STATE'
  | 'GET_SUBSCRIPTION_INVOICES'
  | 'GET_SUBSCRIPTION_INVOICE_RECEIPT'
  | 'GET_SUBSCRIPTION_PRICE'
  | 'GET_ALL_SUBSCRIPTION_PRICES'
  | 'SET_SUBSCRIPTION_PRICE'
  | 'SET_DEFAULT_SUBSCRIPTION_PRICE'
  | 'CREATE_SUBSCRIPTION_PAYMENT_REQUEST'
  | 'GET_SUBSCRIPTION_PAYMENT_REQUESTS'
  | 'REQUEST_SUBSCRIPTION_PUSH_OTP'
  | 'SEND_SUBSCRIPTION_PUSH_PAYMENT_LINK'
  | 'CONFIRM_SUBSCRIPTION_PAYMENT'
  | 'CANCEL_SUBSCRIPTION_PAYMENT'
  // geography (states / counties / locations / villages)
  | 'GET_STATES'
  | 'CREATE_STATE'
  | 'GET_COUNTIES'
  | 'CREATE_COUNTY'
  | 'GET_LOCATIONS'
  | 'CREATE_LOCATION'
  | 'GET_VILLAGES'
  | 'CREATE_VILLAGE'
  | 'BULK_UPLOAD_GEO_NODES'
  | 'UPDATE_GEO_NODE'
  | 'DELETE_GEO_NODE'
  | 'TOGGLE_GEO_NODE_STATUS'
  // vouchers
  | 'CREATE_VOUCHER'
  | 'BULK_ISSUE_VOUCHERS'
  | 'GET_VOUCHERS'
  | 'GET_VOUCHER'
  | 'VOID_VOUCHER'
  | 'REDEEM_VOUCHER'
  | 'SYNC_VOUCHERS'
  | 'VOUCHER_SUMMARY'
  // payroll
  | 'REQUEST_PAYROLL_OTP'
  | 'GENERATE_PAYROLL'
  | 'APPROVE_PAYROLL'
  | 'REJECT_PAYROLL'
  | 'REJECT_PAYROLL_ITEMS'
  | 'DISBURSE_PAYROLL'
  | 'GET_PAYROLL'
  | 'GET_PAYROLLS'
  | 'SET_PAYMENT_APPROVAL_POLICY'
  // payments
  | 'GET_PAYMENTS'
  | 'GET_PAYMENT'
  | 'UPDATE_PAYMENT_STATUS'
  | 'PAYMENT_SUMMARY'
  | 'PAY_PAYMENT_ONLINE'
  // dashboard
  | 'DASHBOARD_METRICS'
  | 'DASHBOARD_PAYMENTS_CHART'
  | 'DASHBOARD_HOUSEHOLDS_CHART'
  // attendance
  | 'GET_ATTENDANCE'
  | 'RECORD_ATTENDANCE'
  // biometric / Android (rarely called from the web dashboard, listed for completeness)
  | 'UPLOAD_HOUSEHOLD_BIO'
  | 'UPLOAD_ALTERNATE_BIO'
  | 'ENROLL_FINGERPRINT'
  | 'VERIFY_FINGERPRINT'
  | 'SYNC_FINGERPRINTS'
  | 'ENROLL_FACE'
  | 'SYNC_FACES'
  | 'UPLOAD_IMAGE'
  | 'SYNC_IMAGES'
  | 'SYNC_PAYMENTS'
  | 'RECORD_FIELD_PAYMENT'
  | 'BIOMETRIC_LOGIN'
