...
public class VoucherDao {
    ...
    /** Redemptions the server explicitly rejected on sync (voided/expired/already-redeemed by
     *  the time this device caught up) -- see SyncManager#syncVoucherRedemptions. Distinct from
     *  {@link #listPendingRedemptions()} so a rejected one is never silently retried forever. */
    public List<Voucher> listFailedRedemptions(){return query("v.redemption_sync_status=?",new String[]{String.valueOf(DatabaseHelper.SYNC_FAILED)});}
    ...
    /** Stops the row from being retried by {@link #listPendingRedemptions()} and records why, so
     *  the agent sees a reason instead of a redemption that just silently never confirms. */
    public void markRedemptionFailed(String code,String error){ContentValues v=new ContentValues();v.put("redemption_sync_status",DatabaseHelper.SYNC_FAILED);v.put("redemption_sync_error",error);helper.getWritableDatabase().update("vouchers",v,"voucher_code=?",new String[]{code});}
    public static class Voucher {public String code;public String householdNumber;public String householdName;public double amount;public String purpose;public String expiresAt;public String status;public String matchedFingerprintUuid;public String latitude;public String longitude;public int redemptionSyncStatus;public String redemptionSyncError;}
}
