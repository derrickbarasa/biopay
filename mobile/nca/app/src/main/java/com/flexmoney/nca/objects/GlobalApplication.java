...
public class GlobalApplication extends Application {
    ...
    public String[] replaceStringArray(String[] str, String nomineeName) {
        int length = str.length;
        String[] newStr = new String[length];
        for (int i = 0; i < length; i++) {
            newStr[i] = str[i].replace("firstNominee", nomineeName);
        }
        return newStr;
    }
    ...
    public boolean ageCheck(TextInputEditText editText, String errorMessage) {
        editText.setError(null);
        String text = editText.getText().toString().trim();
        int no = !text.isEmpty() ? Integer.parseInt(text) : 0;
    ...
    public boolean checkAgeReasonably(MultiAutoCompleteTextView mactv, TextInputEditText editText) {
        String reason = mactv.getText().toString().trim();
        if (reason.isEmpty()) {
            mactv.setError(null);
            return false;
        } else {
            int ageCheck = Integer.parseInt(editText.getText().toString().trim());
            mactv.setError(null);
            editText.setError(null);
            if (reason.contains("Elderly-headed") && ageCheck < 65) {
                editText.setError(ageCheck + " yrs old cannot be Elderly-headed");
                return false;
            }
            if (reason.contains("Child-headed") && ageCheck > 18) {
                editText.setError(ageCheck + " yrs old cannot be Child-headed");
                return false;
            }
            if (reason.contains("18–35") && ageCheck < 18) {
                editText.setError(ageCheck + " yrs old cannot be Households headed by young men and women between the ages of 18–35");
                return false;
            }
            if (reason.contains("18–35") && ageCheck > 35) {
                editText.setError(ageCheck + " yrs old cannot be Households headed by young men and women between the ages of 18–35");
                return false;
            }
        }
    ...
    public boolean isAutoCompleteTextViewSelected(AutoCompleteTextView actv, String errorMessage) {
        String text = actv.getText().toString().trim();
        actv.setError(null);
        if (text.length() == 0) {
            actv.setError(errorMessage);
            return false;
        }
        return true;
    }

    public boolean isMultiAutoCompleteTextViewChecked(MultiAutoCompleteTextView mactv, String errorMessage) {
        String text = mactv.getText().toString().trim();
        mactv.setError(null);
        if (text.length() == 0) {
            mactv.setError(errorMessage);
            return false;
        }
        return true;
    }

    public List sequenceNumbers(int numbers) {
