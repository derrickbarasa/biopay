...
import com.alphabank.dca.adapters.RecyclerHouseholdAdapter;
...
public class HouseholdActivity extends AppCompatActivity {
    ...
    RecyclerHouseholdAdapter adapter;
    private RecyclerView rvHousehold;
    private LinearLayoutManager lmHousehold;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        lmHousehold = new LinearLayoutManager(this);
        beneficiaryList = new ArrayList<>();
        rvHousehold = findViewById(R.id.rvHousehold);

        adapter = new RecyclerHouseholdAdapter(HouseholdActivity.this, beneficiaryList);
        rvHousehold.setAdapter(adapter);
        rvHousehold.setLayoutManager(lmHousehold);
        ...
        try {
            ...
            if(cursor != null){
                if (cursor.getCount() != 0)
                {
                    ...
                    GlobalDialogs.error(HouseholdActivity.this, "Payment Households",
            ...
            if (cursor.getCount() != 0) // data exist
            {
                if (cursor.moveToFirst()) {
                    do {
                        ...
                        String amount =cursor.getString(cursor.getColumnIndexOrThrow("pounds"));
                        String enrolmentNumber = cursor.getString(cursor.getColumnIndexOrThrow("enrolment_number"));
                        String householdName = cursor.getString(cursor.getColumnIndexOrThrow("household_name"));
                        String householdNumber = cursor.getString(cursor.getColumnIndexOrThrow("household_number"));
                        Payment payment = new Payment(householdNumber,amount,householdNumber,enrolmentNumber,householdName,status);
                        beneficiaryList.add(payment);
